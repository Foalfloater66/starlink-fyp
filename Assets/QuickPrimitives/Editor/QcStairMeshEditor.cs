...
public class QcStairMeshEditor : UnityEditor.Editor
{
    ...
    {
        var mesh = target as QcStairMesh;
               ...
               new EditorGUILayout.FadeGroupScope(Convert.ToSingle(!mesh.properties.spiral)))
        {
            if (group.visible == false)
            {
                ...
                mesh.properties.innerRadius =
                    EditorGUILayout.Slider("Radius", mesh.properties.innerRadius, minRadius, 10.0f);
                ...
                mesh.properties.windingDirection =
                    (QcStairMesh.QcStairProperties.WindingDirection)EditorGUILayout.EnumPopup("Winding Direction",
                        mesh.properties.windingDirection);
            }
            else
            {
                                                                   ...
                                                                   QcStairMesh.QcStairProperties.Types.Open)))
        {
            ...
            {
                var stepHeight = mesh.properties.height / mesh.properties.steps;
                float stepDepth;
                if (!mesh.properties.spiral)
                    stepDepth = mesh.properties.depth / mesh.properties.steps;
                else
                    stepDepth = (mesh.properties.innerRadius + mesh.properties.width) * (float)Math.PI /
                        mesh.properties.steps * mesh.properties.rotations;

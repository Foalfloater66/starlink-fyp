...
namespace Routing
{
    ...
    public class Router
    {
            ...
            ScenePainter _painter, Constellation constellation_ctx)
        {
            ...
            Debug.Assert(rn != null, "ExecuteRoute | The last node is empty.");
            Debug.Assert(rn.Id == -1,
                $"ExecuteRoute | The last node isn't -1. Instead, got {rn.Id}");
        ...
        public List<Route> Run(List<Route> routes, AttackTarget target)
        {
            ...
            if (target.Link != null)
            {
                UpdateTargetLinkVisuals(_constellation, target);
            }
